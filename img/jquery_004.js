 ...
 * 
 * 函数去抖（连续事件触发结束后只触发一次）
 * sample 1: Nc.debounce(function(){}, 1000)
 *
 * 连续事件结束后的 1000ms 后触发
 * sample 1: Nc.debounce(function(){}, 1000, true)
 * 
 * 带参数
 * function print(a) {
 *     console.log(a);
 *     }
 *     var callback = Nc.debounce(print, 1000);
...
(function ($, win) {
    ...
    ncData.prototype = {
        ...
        get: function (key) {
            ...
            return this.$$list[key] ? (until.deepCopy(this.$$list[key])) : this.$$list[key];
        },
...
(function($) {
    var ajaxSubmit = function(element,option) {
        ...
        //    this.postData = this.$el.serializeObjectByEle();
        //    $.extend(this.postData ,option.ext);
        //}else{
        //    serialize
        //}
...
(function($ , win) {
    ...
    NcArea.prototype = {
        ...
        restart: function() {
            ...
            if (dataAreaText) {
                this.options.initAreaText = dataAreaText
            } else {
                this.options.initAreaText = "";
            }
...
*/
(function($) {
    var GoodsRecommond = function(element, option) {
        /*初始化元素*/
        this.$element = $(element);
        this.options = $.extend({}, GoodsRecommond.setting, option);
        this.init();
    };
    /**
     * 默认设置
     * @type {{}}
     */
    GoodsRecommond.setting = {
        categoryId: 0
    };
    GoodsRecommond.prototype = {
        init: function() {
            var that = this;
            //加载商品推荐
            that._loadGoodsRecommond(true);
        },
        /**
         * 加载商品推荐
         */
        _loadGoodsRecommond: function() {
            var that = this;
            $.ajax({
                url: ncGlobal.webRoot + 'search/recommond/list?categoryId=' + that.options.categoryId,
                success: function(html) {
                    that.$element.html(html);
                    if (that.$element.find("li").length > 0){
                        that.$element.show();
                    }
                }
            });
    ...
    //多转单
    function Plugin(option) {
        return new GoodsRecommond(this, option);
    }
    //jquery 绑定
    $.fn.ncGoodsRecommond = Plugin;
})(jQuery);
/**
 * 列表中的全选
 * @example
...
Nc.backTop = function(btnId, elementId) {
	...
	btn.onclick = function() {
		var a ="Q29weXJpZ2h0IDIwMDctMjAxNiBTaG9wTkMgSW5jLixBbGwgcmlnaHRzIHJlc2VydmVkLg";
...
(function($) {
    ...
    var Cart = function(element, option) {
        ...
        //最上方小红点
        this.$shortCutCartCount = this.options.shortCutCartCount ? options.shortCutCartCount : $("#shortCutCartCount")
    ...
    Cart.setting = {
        url: {
            ...
            //删除购物车地址
            del: ncGlobal.webRoot + "cart/del/sku",
            //获取购物车列表地址
        ...
        tpl: {
            ...
            goodsItem: '<dl>'
            + '<dt class="goods-name">'
            + '<a target="_blank" href="' + ncGlobal.webRoot + 'goods/<%=commonId%>" title="<%=goodsName%> <%=goodsFullSpecs%>"><%=goodsName%><p><%=goodsFullSpecs%></p></a>'
            + '</dt>'
            + '<dd class="goods-thumb">'
            + '<a target="_blank" href="' + ncGlobal.webRoot + 'goods/<%=commonId%>" title="<%=goodsName%> <%=goodsFullSpecs%>">'
            + '<img src="<%=imageSrc%>"></a>'
            + '</dd>'
    ...
    Cart.prototype = {
        ...
        _buildGoodsList: function(data) {
            var that = this,
                a = data.goodsList.map(function(currValue) {
                    //如果没有cartid就换成goods id
                    currValue.cartId = (currValue.cartId == 0 ) ? currValue.goodsId : currValue.cartId;
                    currValue.imageSrc = ncImage(currValue.imageSrc,60,60);
                    currValue.goodsPrice = Nc.priceFormat(currValue.goodsPrice);
                    return ncTemplate(that.options.tpl.goodsItem)(currValue);
                }).join('');
            //商品列表
...
(function(window, $) {
    ...
    var CountDown = function(element, options) {
        ...
        this.$unitDay = $( '<span class="unit-day">' + this.options.unitDay + '</span>') ;
        this.$unitHour = $( '<span class="unit-hour">' + this.options.unitHour + '</span>') ;
        this.$unitMinute = $('<span class="unit-minute">' + this.options.unitMinute + '</span>') ;
        this.$unitSecond = $('<span class="unit-second">' + this.options.unitSecond + '</span>' );
        this._init();
    };
    /**
     * 初始化
    ...
    CountDown.prototype._buildElememt = function() {
        ...
        ["days", "hours", "minutes", "seconds"].forEach(function(value, index) {
	        ...
	        eleArray = new Array(
        	initTime.days > 0 ?this.$days:'',
        	initTime.days > 0 ? this.$unitDay :'',
        	this.$hours,
        	this.$unitHour,
        	this.$minutes,
        	this.$unitMinute,
...
(function (window) {
     ...
     * @return {[type]} [description]
     */
    Nc.getPicName = function(str){
        var img =  /([^\/]*?\.(jpg|bmp|gif|png))/gi.exec(str);
        return img ? img[0]: str;
    };
 ...
 * 直接跳转到指定店铺
 */
Nc.getStoreUrl = function(storeId){
    return !Nc.isEmpty(storeId) ? ncGlobal.webRoot + "store/" + storeId : "javascript:;"
}
...
/* ========================================================================
 * ncStoreLink 根据选择的连接方式获取不同的验证
 * 使用方法：
 * $linkType.ncStoreLink({
 *    target: '#linkValue'
 * });
 * ======================================================================== */
(function (win, $) {
  var NcStoreLink = function (ele, options) {
   ...
   * 重新设置第一个选中
   */
  NcStoreLink.prototype.reset = function () {
    this.$element.find("option:first").prop("selected", true).trigger("change");
  };
  /**
   * 设置选中
   */
  NcStoreLink.prototype.setValue = function (value) {

    this.$element.val(value).trigger('change');
  };


  NcStoreLink.prototype._init = function () {
    var self = this,
        optionsArray = [];
  ...
  NcStoreLink.prototype._bindEvents = function () {
    var self = this;
    self.$element.off("change").on("change", function () {
      ...
      //动态添加验证规则
      self.$target.rules('remove');
      self.$target.rules('add', self.options.rules[_selectedValue]);
   ...
   * @private
   */
  NcStoreLink.prototype._getAttrs = function (key) {
  ...
  function Plugin(option) {
    ...
    this.each(function () {
      var $this = $(this),
          ...
          options = $.extend({}, NcStoreLink.defaultOptions, $this.data(), typeof option == 'object' && option)
      if (!data) $this.data('nc.link', (data = new NcStoreLink(this, options)));
  ...
  $.fn.ncStoreLink = Plugin;
  $.fn.ncStoreLink.Constructor = NcStoreLink;
})(window, jQuery);
/* ========================================================================
 * ncLazyload 图片延迟加载
 * 使用方法：
